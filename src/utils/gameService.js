...
export const makeMove = async (gameId, row, col, symbol, username) => {
  try {
    ...
    if (
      (isPlayer1 && gameData.currentTurn !== 'O') ||
      (isPlayer2 && gameData.currentTurn !== 'X')
    ) {
      throw new Error('Not your turn');
    }

    // Get the game board - handle both old string format and new object format for backward compatibility
    let board;
    if (typeof gameData.gameBoard === 'string') {
      // For backward compatibility with old string format
      board = JSON.parse(gameData.gameBoard);
    ...
    // Convert to array format for winner check
    const boardArray = [newBoard['0'], newBoard['1'], newBoard['2']];
    ...
    const isDraw = boardArray.flat().every(cell => cell !== null) && !winner;
